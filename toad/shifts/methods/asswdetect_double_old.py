...
def construct_detection_ts(
    ...
    gap_counter = np.zeros_like(values_1d)  # to count gaps
    ...
    for length in range(lmin, lmax + 1):
        ...
        if verbose: print(f"length: {length}, sliding_step: {sliding_step}, n_segs: {n_segs}")

        # forward iteration:
        ix0 = 0                                                 # starting index for sliding window
        detection_ts, counter = update_detection_ts(
            ...
            length,
            sliding_step,
            ix0,                # -> all residuals placed at end
        )
        # backward iteration:
        ix0 = n_tot - length - sliding_step * (n_segs - 1)      # starting index for sliding window
        detection_ts, counter = update_detection_ts(
            detection_ts,
            counter,
            values_1d,
            ...
            length,
            sliding_step,
            ix0,                # -> all residuals placed at beginning
        )
