...
def test_mma_build_and_apply_matches_run_consensus_native():
    """Two-phase MMA API should match one-shot run_consensus on native exports."""
    with tempfile.TemporaryDirectory() as tmp:
        paths = setup_and_export_native(Path(tmp), n_runs=3)
        mma_one_shot = MMA(paths, nside=None)
        mma_two_phase = MMA(paths, nside=None)

        ds_one = mma_one_shot.run_consensus(
            min_consensus=0.5,
            temporal_tolerance=0,
            spatial_tolerance=1,
            min_cluster_area=2,
        ...
        support = mma_two_phase.build_consensus(
            temporal_tolerance=0,
            spatial_tolerance=1,
            show_progress=False,
        )
        ds_two = mma_two_phase.apply_consensus_threshold(
            support,
            min_consensus=0.5,
            min_cluster_area=2,
        )
        ...
        np.testing.assert_array_equal(
            ds_one["consensus_clusters"].values,
            ds_two["consensus_clusters"].values,
        )
        np.testing.assert_allclose(
            ds_one["consensus_clusters_rate"].values,
            ds_two["consensus_clusters_rate"].values,
            equal_nan=True,
        )


def test_mma_build_once_apply_multiple_thresholds_healpix():
    """Repeated apply_consensus_threshold should reuse HEALPix vote accumulation."""
    with tempfile.TemporaryDirectory() as tmp:
        paths = setup_and_export_healpix(Path(tmp), n_runs=3, nside=8)
        mma = MMA(paths, nside=8)
        support = mma.build_consensus(
            temporal_tolerance=0,
            spatial_tolerance=1,
            show_progress=False,
        )
        ds_low = mma.apply_consensus_threshold(
            support,
            min_consensus=0.4,
            min_cluster_area=1,
        )
        ds_high = mma.apply_consensus_threshold(
            support,
            min_consensus=0.8,
            min_cluster_area=1,
        )
        ...
        np.testing.assert_allclose(
            ds_low["consensus_clusters_rate"].values,
            ds_high["consensus_clusters_rate"].values,
            equal_nan=True,
        )
        low_pos = np.sum(ds_low["consensus_clusters"].values >= 0)
        high_pos = np.sum(ds_high["consensus_clusters"].values >= 0)
