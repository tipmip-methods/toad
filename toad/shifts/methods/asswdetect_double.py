...
def construct_detection_ts(
    ...
    gap_counter = np.zeros_like(values_1d)  # to count gaps between segments
...
def update_detection_ts(
    ...
    gradients_fwd = compute_gradients(ix0_fwd, values_1d, times_1d, length, sliding_step)
    # backward iteration:
    n_segs = (n_tot - length) // sliding_step + 1
    ix0_bwd = n_tot - length - sliding_step * (n_segs - 1)      # starting index for sliding window
    gradients_bwd = compute_gradients(ix0_bwd, values_1d, times_1d, length, sliding_step)
