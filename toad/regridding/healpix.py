...
class HealPixRegridder(BaseRegridder):
    ...
    """Regrid data onto a equal-sized HEALPix grid to avoid polar bias in clustering"""
    ...
    def regrid(self, coords: np.ndarray, weights: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
        """
        ...
        if self.nside is None:
            n_pixels = len(np.unique(coords[:, 1])) * len(np.unique(coords[:, 2])) # this assumes that the original grid is a regular grid..
        ...
        unique_hp_indices = self.latlon_to_healpix(unique_coords[:, 0], unique_coords[:, 1])
        
        # Create mapping from lat/lon to healpix index
        coord_to_hp = {(lat, lon): hp_idx 
        ...
        df = pd.DataFrame({
            ...
            'lon': coords[:, 2],
            'vals': weights,
            'hp_pix': [coord_to_hp[(lat, lon)] for lat, lon in zip(coords[:, 1], coords[:, 2])]
        })
    
        # Group and aggregate
        group_cols = ['time', 'hp_pix'] # This means if multiple points fall in the same HEALPix pixel at the same time, they get averaged.
        df = df.groupby(group_cols)['vals'].mean().reset_index()
        
        # Add regridded coordinates
        df['lat'], df['lon'] = zip(*df['hp_pix'].apply(self.healpix_to_latlon)) # Convert healpix index back to lat, lon

        if np.any(np.isnan(df['hp_pix'])):
            logger.warning("Warning: Interpolation contains NaNs. Consider decreasing nside_p")

        self.df_healpix = df
        return np.column_stack([df['time'], df['lat'], df['lon']]), df['vals'].to_numpy()
    ...
    def regrid_clusters_back(self, cluster_labels: np.ndarray) -> np.ndarray:
        ...
        mapping = dict(zip(
            zip(self.df_healpix['time'], self.df_healpix['hp_pix']), # group by both time and hp_pix because hp_pix is constant with time
            cluster_labels
        ))
    ...
    def plot(self, val_var: str = 'cluster', time=None, cmap='coolwarm', center_lon=180):
    ...
    def plot_clusters(self, s=1, cmap=None, color=None, ax=None, extent=None, add_colorbar=True):
        if ax is None:
    ...
    def demo_plot(self):
        ...
        if self.nside is None:
            raise ValueError("Please provide an nside value in the HealPixRegridder constructor for the demo plot.")
