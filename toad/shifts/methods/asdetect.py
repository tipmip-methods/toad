...
class ASDETECT(ShiftsMethod):
    ...
    Steps:
        ...
        6. Normalize the detection array by dividing by the number of window sizes used (or by the counter array for non-centered segmentation methods).
        7. For non-centered segmentation methods, further normalize by downweighting signals at the boundaries where fewer segments overlap, reducing unreliable detections at the start and end of the time series.
...
def construct_detection_ts(
    ...
    For non-centered segmentation methods, the detection signal is further
    normalized to downweight signals at the boundaries where fewer segments
    overlap, reducing unreliable detections at the start and end of the time series.
...
def get_gradients(
    ...
    res = (
        n_tot - segment_length - sliding_step * (n_segs - 1)
...
def get_gradients_double(
    ...
    # backward iteration:
    ix0_bwd = (
        n_tot - segment_length - segment_length * (n_segs - 1)
    ...
    ix0_arr = np.concatenate(
        (np.full(len(gradients_fwd), ix0_fwd), np.full(len(gradients_bwd), ix0_bwd))
    )
